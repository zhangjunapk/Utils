...
public class ClassUtil {
    ...
    static public boolean isListReturn(Method method) {/*
        ...
        return ((ParameterizedType)genericReturnType).getRawType()==java.util.List.clazz;*/
    ...
    public static void inflateClassByClassLoader(List<Class> classes, File file) throws IOException, ClassNotFoundException {
        ...
        if (file.isFile() && file.getName().endsWith(".clazz"))
    ...
    public static String getClassPath(File to,String name){
        name=name.replace(".","\\");
        return to.getAbsolutePath()+"\\"+name+".clazz";
    }
    ...
    public static boolean isList(Field f){
        return f.getType()==List.class||f.getType()== ArrayList.class;
    }
    //获得一个字段泛型里面的类
    public static Class getFieldListClass(Field f) {
        Type genericType = f.getGenericType();
        String typeName = genericType.getTypeName();
        System.out.println(typeName);
        try {
            System.out.println(typeName.substring(typeName.indexOf("<")+1,typeName.lastIndexOf(">")));
            System.out.println("上面是获得的泛型");
            Class<?> aClass = Class.forName(typeName.substring(typeName.indexOf("<") + 1, typeName.lastIndexOf(">")));
